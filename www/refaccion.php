 ...
 <thead>
  ...
  <?php
   ...
   while ($lol = mysql_fetch_assoc($output)){
    echo '<tr><td>'.$lol["id"].'</td><td>'.$lol["nombre"].'</td><td>'.$lol["descripcion"].'</td><td>'.$lol["unidades_disponibles"].'</td><td>'.$lol["unidades_por_equipo"].'</td><td>$ '.$lol["costo_refaccion"].'</td><td><button type="button" class="btn btn-default" data-toggle="modal" data-target="#actualizarRefaccion" onclick="cargarActualizarRefaccion('.$lol["id"].')"><span class="glyphicon glyphicon-pencil"></span> Modificar Unidad</button> <button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-remove-circle"></span> Eliminar Equipo</button></td></tr>';
   }
  ?>
 </tbody>
</table>
